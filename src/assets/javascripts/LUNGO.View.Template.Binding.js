    ...
    var _bindProperties = function(element, template) {
        ...
        for (var property in element) {
            ...
            if (lng.Core.isOwnProperty(element, property) && element[property] !== null) {
                if (element[property] instanceof Object) {
                    // First level iteration
                    // TODO Recursive algorithm
                    for (var prop in element[property]) {
                        binding_field = new RegExp(BINDING.START + property + '.' + prop + BINDING.END, 'g');
                        template = template.replace(binding_field, element[property][prop]);
                    }
                } else {
